"""
=====================================================================
 stage2_tdl/ncrn/train_ncrn_tdl.py — NCRN Stage II
=====================================================================
 Full NCRN trained on 3GPP TR 38.901 TDL channels (10 scenarios)
 with three-phase SNR curriculum:
   Phase I   (epochs  0-33%):  SNR in [5, 15] dB  (EASY)
   Phase II  (epochs 33-67%):  SNR in [0, 15] dB  (MEDIUM)
   Phase III (epochs 67-100%): SNR in [-5, 15] dB (HARD)

 Architecture: ConvLSTM 128 -> 64 + 1x1 skip + SE attention
   + parameter-free three-regime SNR-aware blending head.
 Target: residual = h_true[-1] - h_noisy[-1].
 Input:  3 channels [real, imag, snr_norm].

 Usage:
   python train_ncrn_tdl.py --array 128
   python train_ncrn_tdl.py --array 64

 Outputs: ../../models/ncrn_tdl_<array>.keras
=====================================================================
"""
...
if __name__ == '__main__':
    ...
    p.add_argument('--array', type=int, default=128, choices=[64, 128])
    args = p.parse_args()
    main(args.array, args.array)
