...
MODEL_PATHS = {
    'sadlcs': 'sa_dlcs_tdl_128.keras',
    'ncrn':   'ncrn_tdl_128.keras',
}
...
# ─── Evaluation pipeline ──────────────────────────────────────
def evaluate_methods(snr_range, n_samples, channel_fn):
    """Returns {method: {'nmse_db', 'nmse_lin', 'mse_lin'}} per SNR."""
    sadlcs_m  = load_model('sadlcs')
    ncrn_m    = load_model('ncrn')
...
def plot_stage2(n_samples):
    print("\n  ─── Stage II TDL averaged, 128×128 ───")

    if not any(os.path.exists(os.path.join(MODELS_DIR, MODEL_PATHS[k]))
               for k in MODEL_PATHS):
        print("  [SKIP] No TDL 128 models found.")
        return

    res = evaluate_methods(SNR_RANGE, n_samples, gen_tdl_random)
    ...
    _save_fig(fig, 'nmse_tdl_128')
    plt.close(fig)
    export_csv(res, SNR_RANGE, os.path.join(CSV_DIR, 'tdl_128.csv'))
...
def plot_per_scenario(group, n_samples):
    ...
    for i, sname in enumerate(sc_in_group):
        ...
        export_csv_perscenario(
            snr_arr, nmse_lin,
            os.path.join(CSV_DIR, f'tdl_{sname}_128.csv'))
...
def main():
    p = argparse.ArgumentParser(
        description='Generate TDL NMSE figures for 128×128 MIMO.')
    ...
    print("=" * 70)
    print("  TDL COMPARISON — NMSE Figures (128×128)")
    ...
    if 'stage2' not in args.skip:
        print("\n[1/5] Stage II — averaged TDL NMSE (128×128)")
        plot_stage2(n_tdl)
